...
const styles: React.CSSProperties = { display: 'block', marginBottom: '100px' };

class Form extends Component<{}, FormState> { // типизация props/state
  state = {
    inputText: '',
    textareaText: '',
  ...
  render() {
    const {
      inputText, textareaText, selectText, showData,
    } = this.state;
    ...
    return (
      <>
        <form>
          ...
          <label htmlFor="textarea" style={styles}>
            Text:
            <textarea
              id="textarea"
              value={textareaText}
          ...
          <button type="button" onClick={this.handleShow}>Show Data</button>
        </form>
        <h2>{name}</h2>
        <h3>{text}</h3>
        <h3>{position}</h3>
